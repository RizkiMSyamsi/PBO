			...
			if(!empty($_POST['form-order'])){
				...
				if($difference->days >= 0){
					...
					if($q){
						...
						$last = mysqli_fetch_array(mysqli_query($db,"SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							...
							if(!empty($product)){
								...
								$ins = mysqli_query($db,"insert into detail_pesanan Values(NULL,'$product[id]','$qty','$last[id]')");
				...
				<hr>
				 <form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kota" required class="form-control">
							<?php
								$kota = mysqli_query($db,"select*from kota");
